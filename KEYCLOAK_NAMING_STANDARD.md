...
- **Escalabilidad**: Preparado para multi-tenant (scopes geográficos y funcionales)
- **Separación**: Realms separan tenants, no los nombres
...
```
tlm-{scope}    → scope puede ser geográfico (mx, pe) o funcional (corp)
```

> **Nota:** Un tenant no es equivalente a un país. Ver [ADR-002](./ARCHITECTURE_DECISIONS.md#adr-002-multi-tenancy-por-realm-jwt).
...
tlm-co       → TLM Colombia
tlm-ec       → TLM Ecuador
tlm-corp     → TLM Corporativo (servicios compartidos cross-scope)
```
...
**Ubicación:** Realm del scope (`tlm-mx`, `tlm-pe`, etc.)
**Pattern:** `{servicio}-{scope}-{ambiente}`
...
**Ubicación:** Realm del scope
**Pattern:** `{servicio}-{scope}-{ambiente}`

**Nota:** Aunque el servicio sea local, incluir el scope facilita escalabilidad futura.
...
4. Kong busca un consumer cuyo `jwt_secret.key` coincida con el `iss` del token (`tlm-mx-realm`)
5. Valida la firma con la clave pública RSA del realm
6. ACL verifica que el consumer tenga el grupo requerido (`sisbon-users`)
7. Si válido, envía petición al backend sisbon
...
```bash
# Mismo plugin JWT — Kong acepta cualquier issuer que tenga consumer registrado
# El ACL plugin restringe qué consumers pueden acceder al servicio
curl -X POST http://localhost:8001/services/gestal-prod/plugins \
  -H "Content-Type: application/json" \
  -d '{
    "name": "acl",
    "config": {
      "allow": ["talenthub-users"]
    }
  }'

# Solo tlm-pe-realm tiene el grupo talenthub-users → solo PE puede acceder
```
...
```
Realm: tlm-pe
├─ gestal-pe-dev ✅ (mantener - credenciales ya enviadas)
├─ gestal-pe-qa  ✅ (mantener - credenciales ya enviadas)
```
...
- [ ] Crear realm si no existe (`tlm-{scope}`)
- [ ] Crear `{servicio}-api` en `tlm-corp` (bearer-only)
- [ ] Por cada scope:
  - [ ] Crear `{servicio}-{scope}-dev` en realm del scope
  - [ ] Crear `{servicio}-{scope}-qa` en realm del scope
  - [ ] Crear `{servicio}-{scope}-prod` en realm del scope
...
- [ ] Crear realm si no existe (`tlm-{scope}`)
- [ ] Crear `{servicio}-api` en realm del scope
- [ ] Crear `{servicio}-{scope}-dev`
- [ ] Crear `{servicio}-{scope}-qa`
- [ ] Crear `{servicio}-{scope}-prod`
...
| Consumidor Corp               | tlm-{scope} (Keycloak) | `{servicio}-{scope}-{env}`        | `sisbon-mx-dev`         |
| API Local                     | tlm-{scope} (Keycloak) | `{servicio}-api`                  | `gestal-api`            |
| Consumidor Local              | tlm-{scope} (Keycloak) | `{servicio}-{scope}-{env}`        | `gestal-pe-dev`         |
| Consumidor Local (multi-tipo) | tlm-{scope} (Keycloak) | `{servicio}-{tipo}-{scope}-{env}` | `gestal-mobile-pe-dev`  |
| Client Integración Externa    | tlm-{scope} (Keycloak) | `{servicio}-ext-{partner}`        | `gestal-ext-ats`        |
...
- v1.2 (2026-03-22): `{pais}` → `{scope}` en todos los patrones; corregido flujo de validación Kong; actualizada sección migración
