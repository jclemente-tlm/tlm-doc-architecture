...
| `{env}`     | `dev`, `qa` o `prod`. **Requerido en todos los ambientes.**                         |
...
| `sisbon-mx-prod` | `tlm-mx` | Producción |
| `gestal-pe-dev`  | `tlm-pe` | Desarrollo |
| `gestal-pe-qa`   | `tlm-pe` | QA         |
| `gestal-pe-prod` | `tlm-pe` | Producción |
...
```
# Por canal de acceso
gestal-app-pe-prod       → Backend/web — producción
gestal-mobile-pe-prod    → App móvil — producción
gestal-batch-pe-dev      → Proceso batch — desarrollo

# Por componente del sistema
siata-procservice-pe-prod  → Servicio de procesamiento — producción
...
A diferencia de los clients M2M, las herramientas SSO tienen **un único client por instalación**, no uno por ambiente. La separación de ambientes se logra a través de instalaciones independientes de la herramienta (cada una con su propia URL de redirect), no duplicando el client en Keycloak.
...
| `ext-nomina360-ec-prod` | `tlm-ec` | Producción |
...
| `{sistema}-{scope}-prod` | `tlm-{scope}` | `confidential` | Producción | `sisbon-mx-prod` |
...
```
Client: {sistema}-{scope}-{env} (tlm-{scope})
→ Mappers → Add Mapper → Tipo: Audience
   Included Custom Audience: {sistema}-api
   Add to access token: ON
```

Repetir para cada consumidor de todos los scopes (dev, qa y prod). Esto fuerza que el claim `aud` del JWT incluya `{sistema}-api`, que es lo que el API Gateway verifica.
:::
...
| `{sistema}-{scope}-prod`  | `tlm-{scope}` | `confidential` | Producción | `gestal-pe-prod` |
...
| `ext-nomina360-ec-prod` | `tlm-ec`   | `confidential` | Producción |
...
**Ejemplos:** `ext-talenthub-ats-prod` (producción) · `ext-talenthub-ats-dev` (desarrollo) — el API Gateway routea hacia la API de TalentHub.
...
- [ ] Por cada scope: crear `{sistema}-{scope}-dev`, `{sistema}-{scope}-qa`, `{sistema}-{scope}-prod`
...
- [ ] Crear `{sistema}-{scope}-dev`, `{sistema}-{scope}-qa`, `{sistema}-{scope}-prod`
...
- [ ] Crear `ext-{partner}-{scope}-dev`, `ext-{partner}-{scope}-qa`, `ext-{partner}-{scope}-prod` en `tlm-{scope}`
...
| Entidad                        | Patrón                                                                                                                            | Ejemplo prod             | Ejemplo dev/qa             |
| ------------------------------ | --------------------------------------------------------------------------------------------------------------------------------- | ------------------------ | -------------------------- |
...
| Consumidor M2M                 | `{sistema}-{scope}-{env}`                                                                                                         | `gestal-pe-prod`         | `gestal-pe-qa`             |
| Consumidor M2M multi-tipo      | `{sistema}-{tipo}-{scope}-{env}` — `{tipo}` puede ser canal (`app`, `mobile`, `batch`) o componente (`procservice`, `procworker`) | `gestal-mobile-pe-prod`  | `gestal-mobile-pe-dev`     |
| Consumidor M2M B2B             | `ext-{partner}-{scope}-{env}` en `tlm-{scope}`                                                                                    | `ext-nomina360-ec-prod`  | `ext-nomina360-ec-qa`      |
...
- **MUST** incluir sufijo de ambiente (`dev`, `qa` o `prod`) en todos los clients confidential M2M.
...
- **SHOULD** usar el patrón base (`{sistema}-{scope}-{env}`) como opción por defecto para consumidores M2M internos.
- **SHOULD** escalar al patrón multi-tipo (`{sistema}-{tipo}-{scope}-{env}`) solo cuando existan 2 o más tipos de consumidores con permisos o rate limits distintos, o 2 o más componentes del sistema con necesidad de secretos independientes.
...
- **MUST NOT** incluir scope ni sufijo de ambiente en el nombre de herramientas SSO (`{herramienta}`); cada instalación de la herramienta tiene su propio client — el nombre identifica la herramienta, no el ambiente.
