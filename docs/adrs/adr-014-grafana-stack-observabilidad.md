...
| **Adopción**                         | ✅ Alta (Grafana + stack moderno)              | ✅ Muy alta (estándar logging OSS)            | ✅ Muy alta (25K+ clientes)                   | ✅ Muy alta (analytics líder)             | ✅ Alta (AWS monitoring std)            |
...
| **Integración .NET**                 | ✅ `OpenTelemetry.*` (.NET 6+)                 | ✅ `Serilog.Sinks.Elasticsearch` (.NET 6+)    | ✅ `Datadog.Trace` (.NET 6+, APM completo)    | ✅ `Splunk.Logging.*` (.NET 6+)           | ✅ `AWSSDK.CloudWatch` (.NET 6+)         |
| **Multi-tenancy**                    | ✅ Orgs + data sources                         | ⚠️ Indices/Spaces                             | ✅ Nativo (orgs/teams)                        | ✅ Tenants nativos                        | ⚠️ Por accounts                         |
| **Latencia queries**                 | ✅ `<100ms` (p95)                              | ⚠️ `100-200ms` (requiere tuning)              | ✅ `<200ms` (p95)                             | ✅ `<150ms` (p95)                         | ✅ `<500ms` (p95)                       |
...
- **Operación self-managed:** mitigado con Terraform, Helm charts oficiales y equipo SRE capacitado
- **Curva de aprendizaje Mimir/Tempo/Alloy:** mitigado con documentación de Grafana Labs y training interno
- **Alta disponibilidad compleja:** mitigado con despliegue multi-AZ y S3/GCS como backend
