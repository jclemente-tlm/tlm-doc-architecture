...
| **Integración .NET**        | ✅ `Keycloak.AuthServices.*` (.NET 6+)                | ✅ `Auth0.AspNetCore.Authentication` (.NET 6+)      | ✅ `AWSSDK.CognitoIdentityProvider` (.NET 6+)      | ✅ `Azure.Identity` + `MSAL` (.NET 6+)           | ⚠️ `Google.Cloud.IdentityPlatform` (limitado, .NET 6+)      |
