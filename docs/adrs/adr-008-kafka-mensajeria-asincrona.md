...
| **Latencia**                   | ✅ p99 `<10ms `                                     | ✅ `<50ms `entrega                              | ✅ `<100ms `entrega                           | ⚠️ `<100ms `variable                        | ✅ `<50ms `entrega                            |
...
| **Persistencia**               | ✅ Log distribuido                                  | ✅ 7+ días por defecto                             | ✅ Persistente                                | ✅ Colas durables                           | ✅ Persistencia nativa                        |
...
| **Schema Registry**            | ✅ Confluent Schema Registry (Avro, JSON, Protobuf) | ⚠️ Manual (no nativo)                           | ⚠️ Implementación manual                         | ❌ No soportado                             | ⚠️ Validación de esquema personalizada          |
...
- **Google Cloud Pub/Sub:** lock-in GCP, sin infraestructura GCP existente, costos por mensaje elevados (US$40/TB ingress + US$120/TB egress), no soporta replay nativo, requiere Dataflow para streaming, at-least-once por defecto
