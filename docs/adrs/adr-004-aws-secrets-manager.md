...
| **Integración .NET**           | ✅ `AWSSDK.SecretsManager` (.NET 6+)                 | ✅ `Azure.Security.KeyVault.Secrets` (.NET 6+)     | ⚠️ `Google.Cloud.SecretManager` (.NET 6+)            | ⚠️ Solo CI/CD (.NET 6+)            | ✅ `VaultSharp` (.NET Standard 2.0+)          |
